...
alt_32 timer_main_period_ms = 100;
...
void process_buffer(char* text, int uart_fd) {
    ...
    if (text == NULL) {
		...
		id_switch &= (0b1);
        sprintf(message, "ID %d\n", id_switch+1); // id + 1 because we are using ID 1/2 not 0/1
        write(uart_fd, message, strlen(message));
    }
        ...
        // ^^ UART write buffer max 64 bits so msg needs to be split up and sent seperately.
    }
